...
class JobShopEnv(gym.Env):
    def __init__(self, instance_path=None):
        """
        - actions correspond to a job allocation + one action for no allocation at this time step (NOPE action)
        - keep a time with next possible time steps
        - each time we allocate a job, the end of the job is added to the stack of time steps
        - if we don't have a legal action (i.e. we can't allocate a job), we automatically go to the next time step until we have a legal action
        """

        # values for the instance
        self.jobs = 0 # number of jobs
        self.machines = 0 # number of machines
        self.instance_matrix = None # adjacency matrix of the instance?
        self.jobs_length = None 
        self.max_time_op = 0 # maximum time for an operation
        self.max_time_jobs = 0 # maximum time for a job
        self.number_legal_actions = 0 # number of legal actions
        self.number_machine_legal = 0 # number of legal machines


        # These variables are used for solving the instance.
        # They are re-initialized every time we reset the environment
        self.solution = None
        ...
        #############################################################
        #   Loading the instance. Refactor: Begin!
        #############################################################
        ...
        #############################################################
        #   Loading the instance. Refactor: End!
        #############################################################
        ...
        # check the parsed data is correct
        ...
        """
        matrix with the following attributes/features for each job:
            -Legal job
    ...
    def increase_time_step(self):
        """
        We need to increase every counter when we have a nope action called
            ...
            if was_left_time > 0:
                ...
                if self.time_until_finish_current_op_jobs[job] == 0:
                    ...
                    if self.todo_time_step_job[job] < self.machines:
                        ...
                        # this allows to have 1 is job is over (not 0 because, 0 strongly indicate that the job is a
                        # good candidate)
